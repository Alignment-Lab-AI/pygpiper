...
def split_conversations(data, max_tokens, min_tokens, tokenizer, static_keys, keys_to_count):
    static_token_count = count_tokens({key: data[key] for key in static_keys if key in data}, tokenizer, static_keys)
    ...
    if current_conv and current_token_count >= min_tokens:
        print(f"Yielding final part at token count: {current_token_count}. Split index: {split_index}")
        yield current_conv
...
def process_files(input_dir, output_dir, config, batch_size=1000):
    ...
    for file_name in all_files:
        ...
        for i, conv_part in enumerate(split_conversations(data, max_tokens, min_tokens, tokenizer, static_keys, keys_to_count)):
